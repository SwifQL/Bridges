...
public protocol Bridgeable: AnyBridge {
    ...
    func pool(_ db: DatabaseIdentifier) -> Pool
    
    func db(_ db: DatabaseIdentifier) -> Database
...
extension Bridgeable {
    ...
    public func pool(_ db: DatabaseIdentifier) -> Pool {
        let pool = pools[db.key] ?? _register(db)
        return pool.pool(for: eventLoopGroup.next())
    }
